        ...
        public static RsaKey GenerateRsaKeys(RsaKeyType type = RsaKeyType.XML, int length = 1024)
        {
            ...
            switch (type)
            {
                case RsaKeyType.PKCS1:
                    return RsaKey ?? (RsaKey = new RsaKey {PrivateKey = rsa.ToPEM_PKCS1(), PublicKey = rsa.ToPEM_PKCS1(true)});
                case RsaKeyType.PKCS8:
                    return RsaKey ?? (RsaKey = new RsaKey {PrivateKey = rsa.ToPEM_PKCS8(), PublicKey = rsa.ToPEM_PKCS8(true)});
                case RsaKeyType.XML:
                    return RsaKey ?? (RsaKey = new RsaKey {PrivateKey = rsa.ToXML(), PublicKey = rsa.ToXML(true)});
                default:
